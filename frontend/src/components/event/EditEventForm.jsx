...
import { CalendarDays, MapPin, Sparkles, Tag, Trash2, Type, X } from 'lucide-react'
      ...
      <div className="max-h-[92dvh] w-full max-w-3xl overflow-y-auto rounded-t-2xl border border-purple-100 bg-white shadow-2xl sm:max-h-[90vh] sm:rounded-2xl">
        <div className="flex items-start justify-between border-b border-purple-100 bg-gradient-to-r from-purple-700 to-indigo-700 p-4 text-white sm:p-6">
          <div>
            ...
            <h2 className="mt-1 text-xl font-semibold sm:text-2xl">Re-shape This Event</h2>
            <p className="mt-1 text-xs text-purple-100 sm:text-sm">
              Keep details tidy so your team gets clean task and calendar views.
            </p>
          </div>
          <button
            type="button"
        ...
        <form onSubmit={handleSubmit} className="space-y-5 p-4 pb-6 sm:p-6 sm:pb-6">
          ...
          <div className="flex flex-col gap-2 border-t border-gray-200 pt-5 sm:flex-row sm:flex-wrap sm:gap-3">
            ...
            >
              {loading ? 'Saving updates...' : 'Save Event'}
            </button>
            <button
              type="button"
              onClick={handleDelete}
              disabled={deleteLoading}
              className="inline-flex min-h-[44px] w-full touch-manipulation items-center justify-center gap-2 rounded-lg bg-red-600 px-4 py-2.5 font-medium text-white transition hover:bg-red-700 disabled:opacity-50 sm:w-auto sm:py-2"
            >
